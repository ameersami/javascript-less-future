...
export default () => {
  ...
  const [animationDirection, animationDirectionSetter] = useState<'previous' | 'next' | ''>('');
  ...
  const handleDateRangePickerContainerClick = (e) => {
    e.stopPropagation();
  }
  ...
  const handlePreviousMonthClick = (e) => {
    ...
    monthTextSetter(newCurrentDate.toLocaleString('default', { month: 'long' }));
    yearTextSetter(newCurrentDate.toLocaleString('default', { year: 'numeric' }));
    animationDirectionSetter('previous');
  }
  ...
  const handleToggle = (event) => {
    if (event.newState === 'closed') {
      if (selectedStartDate && selectedEndDate) {
        ...
        selectedStartDateSetter(null);
        selectedEndDateSetter(null);
        hasSelectedValueSetter(false);
      }
    }
  }

  return (
    <div className={styles.dateRangePickerContainer} onClick={handleDateRangePickerContainerClick}>
      <button
        popoverTarget="dateTimePickerPanel"
